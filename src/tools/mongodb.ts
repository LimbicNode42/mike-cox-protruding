...
export function registerMongoTools(
  ...
  server.registerTool(
    ...
    async (args: { database: string; collection: string; filter_query?: string; limit?: number }) => {
      try {
  ...
  server.registerTool(
    ...
    async (args: { database: string; collection: string; filter_query: string; update_query: string }) => {
      try {
        const sessionId = 'default';
        const session = await getOrCreateSession(sessionId, config, sessions);
