...
def register_postgres_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    def postgres_query(sql: str, database: str = "postgres") -> str:
        ...
        try:
            results = db_manager.query(database, sql)
    ...
    @mcp.tool()
    def postgres_execute(sql: str, database: str = "postgres") -> str:
        ...
        if not db_manager:
            return "PostgreSQL is disabled in the server configuration"
        try:
            result = db_manager.execute(database, sql)
    ...
    @mcp.tool()
    def postgres_create_table(database: str, table_name: str, columns: str) -> str:
    ...
    @mcp.tool()
    def postgres_create_database(database_name: str) -> str:
        ...
        try:
            ...
            result = db_manager.execute("postgres", sql)
