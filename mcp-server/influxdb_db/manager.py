...
class InfluxDBManager:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.client = await loop.run_in_executor(None, 
            ...
            for bucket in buckets:
                bucket_list.append({
                    ...
                    "retention_rules": [
                        {
                            "type": rule.type,
                            "every_seconds": rule.every_seconds if hasattr(rule, 'every_seconds') else None
    ...
    async def get_measurements(self, bucket: str, start_time: str = "-1h") -> List[str]:
        ...
        try:
            ...
            result = await loop.run_in_executor(None, query_api.query, flux_query, self.org)
            measurements = []
            ...
            try:
                ...
                loop = asyncio.get_event_loop()
                query_api = self.client.query_api()
                result = await loop.run_in_executor(None, query_api.query, fallback_query, self.org)
                ...
                logger.warning(f"Failed to get measurements with fallback: {fallback_e}")
                return []
    
    async def get_fields(self, bucket: str, measurement: str, start_time: str = "-1h") -> List[Dict[str, Any]]:
    ...
    async def get_tags(self, bucket: str, measurement: str, start_time: str = "-1h") -> List[Dict[str, Any]]:
    ...
    async def get_tag_values(self, bucket: str, measurement: str, tag_key: str, start_time: str = "-1h") -> List[str]:
    ...
    async def query_data(self, bucket: str, flux_query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if f'from(bucket: "{bucket}")' not in flux_query and 'from(bucket:' not in flux_query:
                ...
                for record in table.records:
                    ...
                    if hasattr(record, 'values') and record.values:
                        for key, value in record.values.items():
                            if key.startswith('_') and key not in ['_time', '_measurement', '_field', '_value']:
                                continue
    ...
    async def get_sample_data(self, bucket: str, measurement: str, limit: int = 10, start_time: str = "-1h") -> List[Dict[str, Any]]:
