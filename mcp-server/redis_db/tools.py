...
def register_redis_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    def redis_execute_command(command: str, *args) -> str:
    ...
    @mcp.tool()
    def redis_delete_key(key: str, database: int = 0) -> str:
        """Delete a Redis key"""
        ctx = mcp.get_context()
        redis_manager = ctx.request_context.lifespan_context.redis_manager
        if not redis_manager:
    ...
    def redis_flush_database(database: int = 0) -> str:
        ...
        try:
            redis_manager.execute_command("SELECT", database)
            result = redis_manager.execute_command("FLUSHDB")
